...
def compute_two_side_p_val(resampled_df, original_D_hat_df, neuron):
    """Compute a two-sided p-value for permutation test
       ...
       Args:
           resampled_df: a Pandas DataFrame of all the rates computed after
           resampling.
           original_D_hat_df: a Pandas DataFrame with a single row of all the
           ACTUAL rate values that were computed for a given set of neurons.
           neuron: the name of the neuron column vector to compute the two-sided
           p valaue for.
...
def non_normal_neuron_classifier(dataframe, resampled_df, real_diff_df, p_value=0.05, threshold=10):
    """Classify neurons as selective or not-selective
       ...
       Args:
           ...
           difference of means
           p_value: the cutoff value for the probability that an effect could
           occur by chance.
           threshold: the minimum required number of events for a neuron to be
...
def compute_two_tailed_test(resampled_df, real_d_df, neuron, kwargs): # behavior_name, high_tail, low_tail):
    ...
    """
    if real_d_df[neuron]['d'] >= np.percentile(resampled_df[neuron], kwargs["high_tail"]):
        return kwargs["behavior1_name"]
    elif real_d_df[neuron]['d'] <= np.percentile(resampled_df[neuron], kwargs["low_tail"]):
        return kwargs["behavior2_name"]

    return "not-selective"

def normal_neuron_classifier(dataframe, resampled_df, real_diff_df, threshold=10, **kwargs):
    ...
    Args:
        ...
        threshold: the minimum required number of events for a neuron to be
        considered classifiable, default is 10.
        behavior_name: the behavior to classify each neuron by, e.g. "Running"
        or "Non-Running"
        high_tail: the cutoff for the upper-tail of the distribution
        low_tail: the cutoff for the lower-tail of the distribution
    ...
    for neuron in resampled_df.columns:
        if not neuron in classified_neurons:
            classified_neurons[neuron] = compute_two_tailed_test(resampled_df, real_diff_df, neuron, kwargs)
