...
def detect_ca_transients_mossy(data, thresh, baseline, t_half, frame_rate):
    """
    Args:
        data = (raw_cell) Ca2 transient data in T x N format, where N = # of
        cells (columns), T = time (rows), raw format
    ...
    Returns:
        cell_transients = T x N matrix with s.d. vlaues for all the timepoints
        of the qualified transients (all zeroes except for transients, red
        trace in fig)
        cell_events = T x N matrix with calcium transient peak events
        (all 0's except for the amplitude value at the peak timepoint of each
        transient, asterik in fig)
        cell_AUC = T x N matrix with calculuc transient area under the curve
        (AUC) values (all zeroes except for the AOC value assigned to the peak
        timepoint of each transient)
        zscored_cell = T x N matrix w/ zscored raw_cell data (blue trac in fig)
    """
